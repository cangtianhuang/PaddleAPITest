#!/usr/bin/env python3
"""
API config 推导脚本：由 seq=1024 + seq=2048 推导任意目标 seq（含 MoE 结构重建）。

文件名：derive_api_seq.py
源数据：api_config_1024.txt, api_config_2048.txt
负载模板：moe_profile.json（由真实 4096/8192 抓取统计而来）
输出：  api_config_derived_<TARGET>.txt
用法：
    python derive_api_seq.py 4096
    python derive_api_seq.py 1048576 api_config_1M.txt

────────────────────────────── 两类行的处理 ──────────────────────────────
配置里的行分两类：

A. 确定类（结构/维度随 seq 仿射变化，可精确推导）
   逐项比对 1024/2048 得到：任一会变的整数 v 关于 seq 仿射 v = α·seq + β。
   锚定在 1024：mult = (TARGET-1024)/1024，v(T) = v1024 + (v2048-v1024)·mult。
   按 (结构签名, 数字位置) 学习 {v2048 -> v_target} 映射改写。
...
B. MoE dispatch 类（router 数据相关，逐 token 路由，不可照抄）
   ...
   是数据相关的，1024 与 2048 原始重叠极低，旧脚本「照抄 2048 骨架」不正确。
   本脚本改为「约束重采样」结构重建：
...
SEQ_SMALL = 1024
SEQ_LARGE = 2048
SEQ_TARGET = int(sys.argv[1]) if len(sys.argv) > 1 else 4096

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
FILE_SMALL = os.path.join(BASE_DIR, f"api_config_{SEQ_SMALL}.txt")
FILE_LARGE = os.path.join(BASE_DIR, f"api_config_{SEQ_LARGE}.txt")

OUTPUT_NAME = sys.argv[2] if len(sys.argv) > 2 else f"api_config_derived_{SEQ_TARGET}.txt"
OUTPUT_PATH = os.path.join(BASE_DIR, OUTPUT_NAME)

assert (SEQ_TARGET - SEQ_SMALL) % SEQ_SMALL == 0, "TARGET 必须是 1024 的整数倍"
MULT = (SEQ_TARGET - SEQ_SMALL) // SEQ_SMALL

RANDOM_SEED = 20240528  # 固定种子 → 可复现
JITTER = 0.04  # 重采样抖动幅度（±4%）

# 真实 padded buffer 取值集合（运行时由骨架填充）。第 2 类缓冲行缩放只动 N 属于此集合者。
# MOE_PADDED_N_RAW 含全部真实 padded 值（用于语义明确的 empty 行数）；
# MOE_PADDED_N 剔除与结构常量重合的值（用于 reshape/slice 等可能是权重的形状参数）。
MOE_PADDED_N = set()
...
def main():
    if not (os.path.exists(FILE_SMALL) and os.path.exists(FILE_LARGE)):
        print(f"错误：源文件缺失\n  {FILE_SMALL}\n  {FILE_LARGE}")
        sys.exit(1)
    ...
    # 第二遍：MoE 结构重建。
    # 注意：MoE block 内的旧值（input_N / padded_N / tpe / 切片边界）必须以
    # 「未经仿射改写的 2048 原始骨架」为准来识别——仿射 pass 可能已把 permute 行里
    # 的 tokens_per_expert、buffer 维度等当作 seq 值缩放过，若据此计算 old_padded_N
    # 会得到错值、导致 block 内标量映射 key 对不上。故传入原始 lines_large。
